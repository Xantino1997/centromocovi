...
const Comunidad = ({ comunidadEstatica, resaltado }) => {
    ...
    <>
      <div className={classNames} id={nombre.toLowerCase()}>
        <img
          ...
          alt={`Imagen de ${comunidadEstatica.nombre}`}
        />
          ...
          {busqueda && comunidades.length > 0 && (
            <>
              ...
              {comunidades.map((comunidad, index) => (
                <Comunidad
                  key={index}
                  comunidadEstatica={comunidad} // Cambiado aquí para pasar la comunidad completa
                  resaltado={true}
                />
              ))}
            </>
